Shader "Hidden/Locked/.poiyomi/Poiyomi 8.1/Poiyomi Toon/81fd75dd607756643ab15319658108e0"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			void applyDissolve(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods, in PoiCam poiCam, in PoiLight poiLight)
			{
				...
				#if defined(PROP_DISSOLVETOTEXTURE) || !defined(OPTIMIZER_ENABLED)
				dissolveToTexture = POI2D_SAMPLER_PAN(_DissolveToTexture, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DissolveToTextureUV*/)], float4(1,1,0,0)), float4(2.11,1.26,0,0)) * float4(poiThemeColor(poiMods, float4(1,0.4802478,0,0.4470588).rgb, (0.0 /*_DissolveTextureColorThemeIndex*/)), float4(1,0.4802478,0,0.4470588).a);
				#else
				dissolveToTexture = float4(1,0.4802478,0,0.4470588);
				#endif
				#if defined(PROP_DISSOLVENOISETEXTURE) || !defined(OPTIMIZER_ENABLED)
				...
				+ (0.0 /*_DissolveAlpha8*/)
				+ (0.0 /*_DissolveAlpha9*/);
				float dds = (0.327 /*_DissolveDetailStrength*/);
				...
				#if defined(PROP_DISSOLVEDETAILNOISE) || !defined(OPTIMIZER_ENABLED)
				float dissolveDetailNoise = POI2D_SAMPLER_PAN(_DissolveDetailNoise, _MainTex, poiUV(poiMesh.uv[(0.0 /*_DissolveDetailNoiseUV*/)], float4(1,1,0.54,2.02)), float4(2.2,2.86,0,0));
				#else
				float dissolveDetailNoise = 0;
				#endif
				if ((1.0 /*_DissolveInvertNoise*/))
				{
				...
				switch((1.0 /*_DissolveType*/))
				{
					...
					{
						da = remap(da, 0, 1, -(0.023 /*_DissolveEdgeWidth*/), 1);
						dissolveAlpha = da;
						...
						edgeAlpha = remapClamped(da + (0.023 /*_DissolveEdgeWidth*/), da, noise) * (1 - dissolveAlpha);
						break;
					}
					...
					case 3: // Spherical
					{
						...
						{
							da = remap(da, 1, 0, -(0.023 /*_DissolveEdgeWidth*/), 1);
						} else {
							da = remap(da, 0, 1, -(0.023 /*_DissolveEdgeWidth*/), 1);
						}
						dissolveAlpha = da;
						...
						if((0.0 /*_SphericalDissolveInvert*/))
						{
							...
							edgeAlpha = smoothstep((0.023 /*_DissolveEdgeWidth*/) + .00001, 0, -normalizedDistance);
						} else {
							dissolveAlpha = (normalizedDistance < 0) ? 1 : 0;
							edgeAlpha = smoothstep((0.023 /*_DissolveEdgeWidth*/) + .00001, 0, normalizedDistance);
						}
				...
				if ((0.023 /*_DissolveEdgeWidth*/) || ((1.0 /*_DissolveType*/) == 2 && (0.1 /*_DissolveP2PEdgeLength*/) != 0))
				{
					edgeColor = tex2D(_DissolveEdgeGradient, poiUV(float2(edgeAlpha, edgeAlpha), float4(1,1,0,0))) * float4(poiThemeColor(poiMods, float4(1,0.6585873,0,1).rgb, (0.0 /*_DissolveEdgeColorThemeIndex*/)), float4(1,0.6585873,0,1).a);
					#ifndef POI_SHADOW
				...
				poiFragData.emission += lerp(0, dissolveToTexture * (1.4 /*_DissolveToEmissionStrength*/), dissolveAlpha) + lerp(0, edgeColor.rgb * (7.5 /*_DissolveEdgeEmission*/), smoothstep(0, 1 - (0.0 /*_DissolveEdgeHardness*/) * .99999999999, edgeAlpha));
				#endif
			}
			#endif
			...
			void calculateShading(inout PoiLight poiLight, inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam)
			{
				...
				else if ((1.0 /*_LightingAdditiveType*/) == 1) // Toon
				{
					...
					#endif
					if ((0.582 /*_LightingAdditiveGradientEnd*/) == (0.0 /*_LightingAdditiveGradientStart*/)) (0.582 /*_LightingAdditiveGradientEnd*/) += 0.001;
					poiLight.rampedLightMap = smoothstep((0.582 /*_LightingAdditiveGradientEnd*/), (0.0 /*_LightingAdditiveGradientStart*/), 1 - (.5 * poiLight.nDotL + .5));
					#if defined(POINT) || defined(SPOT)
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.additiveShadow, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.582 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation;
					#else
					poiLight.finalLighting = lerp(poiLight.directColor * max(min(poiLight.attenuation, poiLight.detailShadow), passthrough), poiLight.indirectColor, smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.582 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5)));
					#endif
					return;
				}
				#endif
				...
				if ((1.0 /*_LightingVertexLightingEnabled*/))
				{
					...
					for (int index = 0; index < 4; index++)
					{
						...
						{
							vertexLighting += lerp(poiLight.vColor[index] * poiLight.vAttenuation[index], poiLight.vColor[index] * (0.5 /*_LightingAdditivePassthrough*/) * poiLight.vAttenuation[index], smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.582 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.vDotNL[index] + .5))) * poiLight.detailShadow;
						}
					}
			...
			float3 applyEmission1(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				{
					emissionColor1 = POI2D_SAMPLER_PAN(_EmissionMap1, _MainTex, poiUV(poiMesh.uv[(0.0 /*_EmissionMap1UV*/)], float4(1,1,0,0)), float4(0,0,0,0)) * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap1*/)).rgb * poiThemeColor(poiMods, float4(1,0.6591685,0,1).rgb, (0.0 /*_EmissionColor1ThemeIndex*/));
				}
				else
				{
					emissionColor1 = UNITY_SAMPLE_TEX2D_SAMPLER(_EmissionMap1, _MainTex, ((.5 + poiLight.nDotV * .5) * float4(1,1,0,0).xy) + _Time.x * (5.0 /*_EmissionCenterOutSpeed1*/)).rgb * lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap1*/)).rgb * poiThemeColor(poiMods, float4(1,0.6591685,0,1).rgb, (0.0 /*_EmissionColor1ThemeIndex*/));
				}
				#else
				emissionColor1 = lerp(1, poiFragData.baseColor, (0.0 /*_EmissionBaseColorAsMap1*/)).rgb * poiThemeColor(poiMods, float4(1,0.6591685,0,1).rgb, (0.0 /*_EmissionColor1ThemeIndex*/));
				#endif
			...
			float4 frag(VertexOut i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				poiFragData.baseColor = lerp(poiFragData.baseColor, dot(poiFragData.baseColor, float3(0.3, 0.59, 0.11)), -((10.0 /*_Saturation*/)) * hueShiftAlpha.b);
