...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        envData.roughness = 1 - (0.5 /*_LightingStandardSmoothness*/);
...
{
    return UNITY_BRDF_PBS(1, 0, 0, (0.5 /*_LightingStandardSmoothness*/), poiMesh.normals[1], poiCam.viewDir, CreateLight(poiMesh.normals[1], detailShadowMap), CreateIndirectLight(poiMesh.normals[1])).xyz;
}
...
float3 calculateNonImportantLighting(float attenuation, float attenuationDotNL, float3 albedo, float3 lightColor, half dotNL, half correctedDotNL)
{
    ...
    {
        return lerp(lightColor * attenuation, lightColor * (0.014 /*_LightingAdditivePassthrough*/) * attenuation, smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.25 /*_LightingAdditiveGradientEnd*/), dotNL)) * detailShadow;
    }
    else //if((0.0 /*_LightingAdditiveType*/) == 2) // Wrapped
    {
...
float3 calculateFinalLighting(inout float3 albedo, float4 finalColor)
{
    float3 finalLighting = 1;
        ...
        else if ((0.0 /*_LightingAdditiveType*/) == 1) // Toon
        {
                ...
                finalLighting = lerp(poiLight.color * max(poiLight.additiveShadow, (0.014 /*_LightingAdditivePassthrough*/)), poiLight.color * (0.014 /*_LightingAdditivePassthrough*/), smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.25 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * poiLight.attenuation * detailShadow;
            #else
                finalLighting = lerp(poiLight.color * max(poiLight.attenuation, (0.014 /*_LightingAdditivePassthrough*/)), poiLight.color * (0.014 /*_LightingAdditivePassthrough*/), smoothstep((0.0 /*_LightingAdditiveGradientStart*/), (0.25 /*_LightingAdditiveGradientEnd*/), 1 - (.5 * poiLight.nDotL + .5))) * detailShadow;
            #endif
        }
        else //if((0.0 /*_LightingAdditiveType*/) == 2) // Wrapped
        {
            ...
            if ((0.0 /*_LightingRampType*/) == 1) // Math Gradient
            poiLight.rampedLightMap = lerp(float4(1,1,1,1), float3(1, 1, 1), saturate(1 - smoothstep((0.0 /*_LightingGradientStart*/) - .000001, (0.5 /*_LightingGradientEnd*/), 1 - uv)));
            float shadowatten = max(poiLight.additiveShadow, (0.014 /*_LightingAdditivePassthrough*/));
