...
void calculateVertexLightingData(in v2f i)
{
		...
		for (int index = 0; index < 4; index++)
		{
			...
			if (float(0) == 1)
			{
				...
				poiLight.vColor[index] = min(poiLight.vColor[index], poiLight.vColor[index] / (intensity / float(0.42)));
			}
	...
	poiLight.occlusion = 1;
		...
		poiLight.color = magic + normalLight;
			...
			if (float(0) == 1)
			{
				...
				poiLight.color = min(poiLight.color, poiLight.color / (additiveLightIntensity / float(0.42)));
			}
		#endif
	#endif
	#ifdef FORWARD_BASE_PASS
