...
void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
{
    ...
    if (float(0.021))
    {
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, edgeColor.rgb, smoothstep(0, 1 - float(0.731) * .99999999999, edgeAlpha));
    }
    dissolveEmission = lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(1.77), smoothstep(0, 1 - float(0.731) * .99999999999, edgeAlpha));
}
#endif
