    ...
    void ApplyPositionText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(-0.2,0,0,0), float(90), float4(-0.9,0.1,1,1), uv);
        ...
        fixed4 textPositionPadding = float4(0,0,0.4,0);
        ...
        float2 msdfUnit = float(4) / float4(0.0009765625,0.0009765625,1024,1024).zw;
        ...
        globalTextEmission += float4(1,1,1,1).rgb * opacity * float(3);
    }
    ...
    void ApplyFPSText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(-0.05,0,0,0), float(90), float4(-0.9,0.2,1,1), uv);
        if(uv.x > 1 || uv.x < 0 || uv.y > 1 || uv.y < 0)
        {
            return;
        }
        ...
        float startUV = 1 / totalCharacters * currentCharacter;
        float endUV = 1 / totalCharacters * (currentCharacter + 1);
        fixed4 textFPSPadding = float4(0,0,0.15,0);
