    ...
    void ApplyPositionText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(0,0.4,0,0), float(0), float4(0.98,0.054,1,1), uv);
        ...
        float2 msdfUnit = float(4) / float4(0.001953125,0.001953125,512,512).zw;
        float sigDist = median(samp.r, samp.g, samp.b) - 0.5;
        sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
        float opacity = clamp(sigDist + 0.5, 0, 1);
        albedo.rgb = lerp(albedo.rgb, float4(1,0.01298303,0,1).rgb, opacity * float4(1,0.01298303,0,1).a);
        globalTextEmission += float4(1,0.01298303,0,1).rgb * opacity * float(20);
    }
    ...
    void ApplyTimeText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(0.3,0,0,0), float(0), float4(1.6,0.22,1,1), uv);
    ...
    void ApplyFPSText(inout float4 albedo, float2 uv)
    {
        ...
        uv = TransformUV(float4(-0.4,-0.3,0,0), float(0), float4(1.6,0.26,1,1), uv);
        ...
        sigDist *= max(dot(msdfUnit, 0.5 / fwidth(uv)), 1);
        float opacity = clamp(sigDist + 0.5, 0, 1);
        albedo.rgb = lerp(albedo.rgb, float4(1,0.01264614,0,1).rgb, opacity * float4(1,0.01264614,0,1).a);
        globalTextEmission += float4(1,0.01264614,0,1).rgb * opacity * float(20);
    }
