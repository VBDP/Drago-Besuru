...
UnityLight CreateLight(float3 normal, fixed detailShadowMap)
{
    ...
    light.color = saturate(_LightColor0.rgb * lerp(1, poiLight.attenuation, float(0.146)) * detailShadowMap);
...
float FadeShadows(float attenuation)
{
            ...
            attenuation = lerp(1, poiLight.attenuation, float(0.146));
        #endif
...
void ApplySubtractiveLighting(inout UnityIndirect indirectLight)
{
        ...
        poiLight.attenuation = FadeShadows(lerp(1, poiLight.attenuation, float(0.146)));
...
UnityIndirect CreateIndirectLight(float3 normal)
{
    ...
    indirectLight.specular = 0;
        ...
        envData.roughness = 1 - float(0.405);
...
{
    return UNITY_BRDF_PBS(1, 0, 0, float(0.405), poiMesh.normals[1], poiCam.viewDir, CreateLight(poiMesh.normals[1], detailShadowMap), CreateIndirectLight(poiMesh.normals[1])).xyz;
}
void calculateBasePassLightMaps()
{
        ...
        shadowStrength = 1;
        #ifndef OUTLINE
            shadowStrength = POI2D_SAMPLER_PAN(_LightingShadowMask, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float(0.377);
        #else
            shadowStrength = float(1);
        #endif
        float bw_lightColor = dot(lightColor, grayscale_vector);
        float bw_directLighting = (((poiLight.nDotL * 0.5 + 0.5) * bw_lightColor * lerp(1, poiLight.attenuation, float(0.146))) + dot(ShadeSH9Normal(poiMesh.normals[1]), grayscale_vector));
        ...
        switch(float(0))
        {
            case 0: // Ramp Texture
            {
                poiLight.rampedLightMap = lerp(1, UNITY_SAMPLE_TEX2D(_ToonRamp, poiLight.lightMap + float(-0.056)).rgb, shadowStrength.r);
            ...
            case 1: // Math Gradient
            {
                ...
                if (float(0))
                {
                    poiLight.finalLighting = lerp((directLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.377)));
                }
                else
                {
                    poiLight.finalLighting = lerp((indirectLighting * shadowColor * poiLight.occlusion), (directLighting), saturate(poiLight.rampedLightMap + 1 - float(0.377)));
                }
            }
            break;
            case 2:
        ...
        if (float(4) == 2) // Wrapped
        {
            ...
            float directatten = lerp(1, poiLight.attenuation, float(0.146));
            uint normalsindex = float(0) > 0 ? 1: 0;
                    ...
                    poiLight.finalLighting = (poiLight.indirectLighting + poiLight.directLighting) * saturate(poiLight.rampedLightMap + 1 - float(0.377));
                }
                if (!float(0))
                {
