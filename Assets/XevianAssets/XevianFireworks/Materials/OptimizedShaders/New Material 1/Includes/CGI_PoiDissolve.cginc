...
float _DissolveAlpha_newmaterial;
...
void calculateDissolve(inout float4 albedo, inout float3 dissolveEmission)
{
    ...
    #if defined(PROP_DISSOLVETOTEXTURE) || !defined(OPTIMIZER_ENABLED)
        dissolveToTexture = POI2D_SAMPLER_PAN(_DissolveToTexture, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)) * float4(0,0,0,0);
    #else
    ...
    #endif
    float da = _DissolveAlpha_newmaterial
    + float(0)
    + float(0)
    + float(0)
    + float(0)
    ...
    if (float(1) == 1) // Basic
    {
        ...
        edgeAlpha = remapClamped(noise, da + float(0.092), da, 0, 1) * (1 - dissolveAlpha);
    }
    ...
    if (float(0.092))
    {
        ...
        #endif
        albedo.rgb = lerp(albedo.rgb, edgeColor.rgb, smoothstep(0, 1 - float(0.296) * .99999999999, edgeAlpha));
    }
    dissolveEmission = lerp(0, dissolveToTexture * float(0), dissolveAlpha) + lerp(0, edgeColor.rgb * float(1), smoothstep(0, 1 - float(0.296) * .99999999999, edgeAlpha));
}
#endif
